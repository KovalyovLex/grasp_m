...
for det = 1:inst_params.detectors
    if strcmp(inst_params.(['detector' num2str(det)]).view_position, 'centre')
        ...
        grasp_handles.displayimage.(['menuitems' num2str(det)]) = uimenu(grasp_handles.displayimage.(['contextmenu' num2str(det)]),'label','Detector On','userdata',det,'callback','main_callbacks(''detector_on'');');
        grasp_handles.displayimage.(['menuitems' num2str(det)]) = uimenu(grasp_handles.displayimage.(['contextmenu' num2str(det)]),'label','Detector Off','userdata',det,'callback','main_callbacks(''detector_off'');');
