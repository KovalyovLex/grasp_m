function [inst_config, inst_component] = ornl_cg2_model_component

inst_config = [];
inst_component = [];

%cg2 & cg3 can translate detector off to side 400mm
...
inst_component(n).length = 0;
inst_component(n).xydim = {[40e-3], [20e-3]};
inst_component(n).value = 1;
inst_component(n).drawdim = [0, 100e-3];
inst_component(n).drawcntr = [0, 0];
inst_component(n).color = [1, 1, 0];
...
inst_config.tube_length = 21; %m

%Rear Detector (1) 
n=n+1;
inst_component(n).name = 'Detector 1: Rear';
inst_component(n).position = 3;
inst_component(n).parameters.position_max = 19.5;
